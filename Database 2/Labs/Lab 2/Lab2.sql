...
select employees.first_name, employees.last_name, (employees.first_name)||' '|| (employees.last_name) as emp_name,
employees.job_id, departments.manager_id, departments.department_id, locations.city, countries.country_name from 
employees
...
--2
select employees.first_name, count(JOB_HISTORY.employee_id) as previous_jobs
from employees
Join  job_history on job_history.employee_id = employees.employee_id group by employees.first_name;
--3
select employees.first_name
from employees
Join  job_history on job_history.employee_id = employees.employee_id 
having count(JOB_HISTORY.employee_id) > 1
group by employees.first_name;
--4 a
select first_name
from employeeInfo
where employeeInfo.country_name = 'United States of America';
--4 b
select first_name
from employeeInfo
where employeeInfo.country_name = 'United Kingdom';
-- 4 c
select first_name
from employeeInfo
where employeeInfo.country_name = 'United Kingdom' OR employeeInfo.country_name = 'United States of America';
-- 4 d 
select first_name
from employeeInfo
where employeeInfo.country_name = 'United Kingdom'
...
select * 
from departments
WHERE departments.department_id = (select employees.department_id from employees where employees.employee_id = null);