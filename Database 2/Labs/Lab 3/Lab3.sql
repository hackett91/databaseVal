...
select facilitator_name, organizer_name 
from CD_FACILITATOR
JOIN CD_SESSION_RUNS ON (CD_SESSION_RUNS.facilitator_id = CD_FACILITATOR.facilitator_id)
JOIN CD_CODING_SESSION ON (CD_CODING_SESSION.s_start = CD_SESSION_RUNS.s_start)
JOIN CD_ORGANIZER ON ( CD_ORGANIZER.organizer_id = CD_CODING_SESSION.organizer_id);

-- Q4 Write a query to eturn a list  name and email address o ele who have registered as
-- facilitators but wo never egisterd as orgainers. ( you can assume that the email address
-- are unique t the person
...
-- write sql to list the names o any attendee tat attended all session runs falitated by ken 
...
WHERE EVENTS.facilitator_id = CD_FACILITATOR.facilitator_id
AND EVENTS.coder_id = CD_ATTENDEE.coder_id);
