...
--CONSTRAINT markup_chk CHECK (markup < 100.00),
...
ALTER TABLE Product ADD markup NUMBER(4,2); 
...
-- 5 marks
UPDATE Product 
SET markup = (retailPrice - costPrice)/100 ;

--3. Add a constraint to the markup column so that it cannot contain null
-- 5 marks
ALTER TABLE Product MODIFY markup NOT NULL; 
...
-- 5 marks
UPDATE Product Set drugnondrug = 0
...
-- 10 marks -- NOTE: Is their no need to add not null as a constraint because the check makes sure its either 1 of zero??
ALTER TABLE Product 
ADD CONSTRAINT drugnondrug_ck CHECK (drugnondrug = 0 OR drugnondrug = 1);
ALTER TABLE Product MODIFY drugondrug NOT NULL;
...
-- 10 marks
Update PrescriptionItem set preDosage = 50
where stockCode in (SELECT stockCode FROM Product WHERE stockDescription = 'Calpol');
SELECT * FROM PrescriptionItem ;
--8.
...
-- 15 marks
DELETE FROM PrescriptionItem WHERE prescriptionID in (select prescriptionID from Prescription where custID = (select custID from Customer where custName = 'Danny'));
DELETE FROM (select prescriptionID from Prescription where custID = (select custID from Customer where custName = 'Danny')); 
--9.
-- Change all nondrugsales of Umberella to be for product Lynx. You need to use sub-queries
--10 marks
--UPDATE
UPDATE NonDrugSale SET stockCode = (SELECT stockCode FROM Product WHERE stockDescription = 'Lynx')
WHERE stockCode = (SELECT stockCode FROM Product WHERE stockDescription = 'Umbrella');
...
--You should ensure case sensitivity is not an issue.
-- 15 marks
DELETE FROM NonDrugSale WHERE stockCode =( SELECT stockCode FROM Product WHERE stockDescription = 'Lynx');

DELETE FROM Product WHERE stockDescription = 'Lynx';
...
--You should ensure case sensitivity is not an issue.
-- 15 marks
ALTER TABLE Prescription ADD notes VARCHAR2(100);

UPDATE Prescription 
SET notes = 
CASE WHEN  docID = (SELECT docID FROM Doctor WHERE docName = 'Lindsey') OR docID = (SELECT docID FROM Doctor WHERE docName = 'Jack')
THEN 'Verify instructions'
WHEN  docID = (SELECT docID FROM Doctor WHERE docName = 'Micheal') 
THEN 'Check with Customer'
ELSE 'Discuss with George'
end;
