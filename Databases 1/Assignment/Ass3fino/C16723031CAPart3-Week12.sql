...
--25 marks
SELECT stockDescription AS "Product", staffName AS "Staff Member"
FROM Product
INNER join
(
...
USING (stockCode)
UNION
SELECT stockDescription, staffName
FROM Product
INNER JOIN
(
    SELECT StockCode, staffName, staffID
...
FROM Customer
WHERE SUBSTR(custName, 0, 1) LIKE 'B'
UNION
SELECT staffName AS "Name"
FROM Staff
WHERE SUBSTR(staffName, 0, 1) LIKE 'B'
UNION
SELECT stockDescription AS "Name"
FROM Product
WHERE SUBSTR(stockDescription,0,1) LIKE 'B';
...
CREATE VIEW DoctorLeagueTable AS
    SELECT docName AS "DoctorName", COUNT(prescriptionID) AS "NumPrescriptions"
    FROM Doctor
    INNER JOIN Prescription
    USING (docID)
    GROUP BY docName
