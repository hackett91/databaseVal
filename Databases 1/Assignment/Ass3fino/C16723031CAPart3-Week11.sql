...
FROM Product 
LEFTOUTER JOIN PrescriptionItem USING(stockCode) 
WHERE drugnondrug = 1 AND prescriptionID IS NULL ;
...
-- 15 marks
select stockcode, decode(count(prescriptionid), 0 , 'None', count(prescriptionid)) from product
left outer join prescriptionitem using (stockcode)
group by stockcode;
...
-- 10 marks
SELECT custID , decode(prescriptionID ,  NULL , 0 ,prescriptionID ) as PrescriptionID From PRESCRIPTION
FULL OUTER JOIN Customer using(custID);
...
-- 20 marks
select custName , decode(docName , null , '0' , docName)as DoctorName ,
decode(prescriptionID,null , '0' , prescriptionID)as prescriptionID FRom Prescription
Inner Join Doctor using(docID)
...
select custName,custAddress , custPhone , decode(docName , null , '0' , docName)as DoctorName ,
decode(prescriptionID,null , '0' , prescriptionID)as prescriptionID FRom Prescription
Inner Join Doctor using(docID)
Right outer join Customer using(custId);

--to verify view was created
...
-- 15 marks
select custName , decode(docName , null , '0' , docName)as DoctorName , decode(prescriptionID , null , '0',prescriptionID) as prescriptionID
FRom Customer
left outer join(select custid, docName ,prescriptionID from Prescription
join Doctor using(docID)) docdetails 
using (custid);
