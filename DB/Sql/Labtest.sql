...
--1	Write SQL to list  member names (MNAME) and BRANDNAMEs they have borrowed more than once.	20 marks
select member.mname, equipmenttype.brandname
from member
...
having count(loan.memberid) > 1
group by (member.mname, equipmenttype.brandname) ;


--2	Write SQL to list the Brandnames of items for loan that have never been borrowed.	20 marks 
select itemforloan.brandname 
from itemforloan
...
select itemforloan.brandname 
from itemforloan
where itemforloan.serialno in ( select 
loan.serialno from loan);


--3	Write SQL to list the names of members who have borrowed iPads (BRANDNAME has iPad in it), but never watches (BRANDNAME has watch in it). 	20 marks

select member.mname
from member 
join loan on loan.memberid = member.memberid
where loan.serialno in (
select itemforloan.serialno from itemforloan
where itemforloan.brandname like '%iPad%'
AND itemforloan.brandname  not like '%watch%' );



--4	Write SQL to list the names of members who have borrowed all equipment types provided by Dell (eqcompany = 'Dell').	20 marks
...
where not exists
( select * from x
where x.brandname = b.brandname
and x.memberid = a.memberid));

--5	Write SQL to list member names (MNAME) and the total penalty amount owed each has incurred for items that have been borrowed but not returned (returndate is null). Remember SYSDATE is the current date.	20 marks
create or replace view notReturned as 
select * from loan
where returndate is null;


select mname, count( select memberid from notReturned)
from member
group by (mname);
